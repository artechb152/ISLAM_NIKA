...
outer:
for (const [x,z] of [[-12,-20],[-12,-28],[-11,-33],[-10,-36],[-14,-33],[-9,-31]]) {
  const p = await goto(page, x, z, { maxIter: 10, tol: 1.5, log: false });
  let t = await T(300);
  const m = t.match(/(\d+) מ׳/);
  console.log('probe', x, z, '->', p ? [p.x, p.z] : null, m ? m[1]+'m' : '-');
  if (t.includes('טוען')) { await page.waitForTimeout(9000); t = await T(300); }
  if (!t.includes('המנזר') && t.length > 60) { console.log('TRANSITIONED:', JSON.stringify(t.slice(0,80))); break outer; }
}
console.log('FINAL:', JSON.stringify(await T(700)));
await shot(page, '268-west-gate-hunt');
await browser.close();
