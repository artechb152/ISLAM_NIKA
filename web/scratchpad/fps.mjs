/* Frame rate in a region after boot, via rAF counting — is the loop alive and how fast? */
...
await wait(3000)
const fps = await pg.evaluate(() => new Promise((res) => { let n = 0; const t0 = performance.now(); const tick = () => { n++; if (performance.now() - t0 < 2000) requestAnimationFrame(tick); else res(n / 2) }; requestAnimationFrame(tick) }))
console.log(region, 'fps ≈', fps)
await browser.close()
