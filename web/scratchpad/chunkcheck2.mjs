...
console.log('NEARLOG lines seen:', got.filter((l) => l.includes('NEARLOG')).length)
let found = { taskprop: [], paint: [] }
for (const u of js) {
  const body = await (await ctx.request.get(u)).text()
  if (body.includes('NEARLOG')) found.taskprop.push(u.split('/').pop())
  if (body.includes('ch1paint')) found.paint.push(u.split('/').pop())
}
console.log('js chunks loaded:', js.size)
console.log('chunks with NEARLOG:', found.taskprop.join(', ') || 'NONE')
