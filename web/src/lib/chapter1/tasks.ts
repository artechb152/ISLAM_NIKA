...
export const TASKS: Task[] = [
  /* מחנה הלילה, התחנה הראשונה: עוגן שפת אי-הוודאות של הפרק כולו. לא
     שואלים "מה כתוב בספרים" — מציבים את השחקן מול חתיכת אבן אמיתית
     ושואלים מה היא מסוגלת להוכיח. ההבחנה חקוק/מסופר נטבעת כאן פעם
     אחת, וכל ההסתייגויות בהמשך רק מהדהדות אותה.

     ── למה כאן ולא ברמות תימן ──────────────────────────────────────
     המרצה כתבה על התחנה ההיא „לא כתבתי את זה בחוברת", וביקשה להעביר
     אותה אל המחנה. אבן חרוטה בשולי מחנה שיירה אינה פשרה אלא דיוק:
     כתובות ערביות קדומות נחרתו בדיוק במקומות כאלה — עצירות לילה
     ובארות — בידי מי שעצר שם ללון.

     ── ושתי שאלות, לא אחת ──────────────────────────────────────────
     למחנה הייתה תחנה משלו (`task-plan-route`, המפה על החול). המנוע
     מחזיק תחנה אחת לאזור, ולכן שאלת המסלול ירדה אל שלב הפירוש של
     התחנה הזאת — אותן שלוש אפשרויות ואותם נימוקים, אחרי שנקבע מה
     האבן מוכיחה. */
  {
    id: 'task-compare',
    region: 'night-camp',
    ...
    /* ── מול העיניים, לא מאחוריהן ─────────────────────────────────
       האבן עמדה ב-(-3, 9.5) — מאחורי נקודת הפתיחה, כלומר מחוץ למסך
       ברגע שהשחקן מקבל את השליטה. החץ אמנם נצמד לשפה ומצביע אחורה
       (נמדד: rotate(180deg) בשפה התחתונה), אבל הדבר הראשון שהפרק
       מבקש לא אמור להיות מאחור. עכשיו היא 4.5 מטר קדימה ו-27° מן
       הציר — בתוך המסגרת מן הפריים הראשון, לצד הדרך שיוצאת מהמחנה. */
    x: -2, z: 0, model: 'none', h: 0.9, ry: 0.4,
    prompt: 'קראו את האבן',
    ...
    /* שאלת המסלול, כלשונה בתחנה שהייתה למחנה. היא אינה עוד גרירה על
       מפה מצוירת — שלב הפירוש הוא פאנל שאלה — אבל שלוש האפשרויות
       והנימוקים שלהן הם אותם שלושה בדיוק. */
    interpret: {
      question:
        'והמפה פרושה על החול לאור המדורה. אנחנו כאן, בדרום — והשיירה ' +
        'יוצאת עם שחר. אמור לי לאן.',
      options: [
        {
          id: 'north',
          ...
          note:
            ...
            'ובקצה שלה יושבים הקונים של הבושם והתבלינים. וכל מה שמחכה לנו: ' +
            'תחנת הגבול, ית׳רב, ומכה.',
        },
        {
          id: 'east',
      ...
      'רשמתי את הכלל הראשון של המסע: מה שחקוק מוכיח; מה שמסופר — נבחן בזהירות. ' +
      'והמסלול סומן: צפונה בדרך הבשמים, אל השווקים שבקצה.',
    source: '§1',
  },
  ...
  /* ── task-plan-route ירדה ────────────────────────────────────────
     תחנת המפה של מחנה הלילה. כשרמות תימן אוחדה לתוך המחנה (15.9.2026)
     נשארו שתי תחנות באזור אחד, והמנוע מחזיק אחת. שאלת „לאן יוצאים עם
     שחר" עברה בשלמותה אל שלב הפירוש של `task-compare` שלמעלה — אותן
     שלוש אפשרויות ואותם נימוקים — ושאלת „למה דווקא היא" נבלעה בנימוק
     של התשובה הנכונה. מה שירד בפועל הוא הגרירה על המפה המצוירת.
     הגרסה המלאה נשמרת בהיסטוריית git של הקובץ הזה. */
]
