...
const QUESTIONS: Q[] = [
  {
    ...
    label: 'סדר התחנות',
    type: 'order',
    /* חמש מתוך שמונה, ולכן „המרכזיות" ולא „התחנות". הניסוח הקודם ביקש
       את סדר המסע והציג שש מתוך תשע — בלי דרך ההעמסה, המנזר והיציאה —
       והמשוב עוד הוסיף „תחנה אחר תחנה". (תשע היו לפני שרמות תימן
       אוחדה לתוך מחנה הלילה.) */
    prompt: 'סדרו את חמש התחנות המרכזיות לפי הסדר שבו נעברו',
    steps: [
      'מחנה הלילה — הכתובת החרותה באבן והמפה על החול',
    ...
    ok:
      'זה הסדר שבו עברנו. השיירה עולה צפונה ממחנה הלילה עד ית׳רב, ומשם פונה בחזרה דרומה אל מכה — ' +
      'שיושבת מדרום לית׳רב. בין אלה עברנו גם בדרך ההעמסה ובמנזר.',
    retry:
