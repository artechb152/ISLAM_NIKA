...
const RAWI_ARRIVALS: Record<string, { gesture: Gesture; text: string }> = {
  ...
  mecca: {
    gesture: 'talk',
    text: 'ירדנו דרומה בחזרה, ומכה לפנינו. הרבה דרכים נפגשות כאן, והרבה אמונות איתן. הסתובב לאט — הכל בעיר הזאת עומד במקומו מסיבה.',
  },
  exit: {
    gesture: 'talk-nod',
...
export default function Game() {
  ...
  const objective =
    talksLeft > 0 && stage !== 'brief'
      ...
      : stage === 'look'
        ? (sightsLeft > 0
            ? `הביטו במה שמאיר — נותרו ${sightsLeft} דברים לראות כאן`
            : 'הרימו ובחנו את מה שמאיר על הקרקע')
        : stage === 'act'
          ? ACT_LINE
          : stage === 'interpret'
  ...
  const hintText = (() => {
    ...
    if (stage === 'look') return 'נשארה כאן עדות שלא נבחנה — חפשו את האור על הקרקע ולחצו E'
