...
/* שלוש דרגות סימון, ולא אחת.
   האלומה האנכית שהייתה כאן נראתה כסמן מדע-בדיוני שהודבק על העולם,
   והיא סימנה בדיוק אותו דבר לכל סוג של פעולה. היא ירדה. מה שנשאר
   הוא אור שיושב בתוך התאורה המקומית, בשלוש עוצמות:

     'next'  היעד הבא, והוא היחיד שנקרא מרחוק: כתם חם ורך שנושם.
     'ready' משהו שאפשר לבדוק עכשיו — נדלק רק בקרבה או תחת הסמן,
             ובעוצמה שהיא כמעט לחישה.
     'done'  נעשה. אין אור בכלל.
...
function GroundGlow({ x, z, r = 1.1, live, tone = '#f0c877', rank = 'ready' }: {
  ...
  /** דעיכה רכה כשהדרגה מתחלפת — היעד לא נדלק ולא נכבה בבת אחת */
  ...
  useFrame(({ clock }, dt) => {
    ...
    mat.current.opacity = k * (0.08 + near * near * 0.5 + breathe * (0.04 + near * 0.12))
    if (ring.current) ring.current.opacity = k * (0.08 + near * 0.26)
    const sc = 1 + near * (rank === 'next' ? 0.2 : 0.1) + breathe * 0.05
    m.current.scale.set(sc, sc, sc)
    m.current.visible = k > 0.01
  })
  ...
  return (
    <group>
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[x, y + 0.02, z]} renderOrder={1}>
        ...
        <meshBasicMaterial ref={ring} color="#3a2c1c" transparent opacity={0} depthWrite={false} />
      </mesh>
...
function LampReveal({ live, target, home, onRevealed, revealed, active }: {
  ...
  useEffect(() => {
    ...
    const key = (e: KeyboardEvent) => {
      if (doneRef.current || !activeRef.current) return
      /* ⚠ כאן עמד `if (!live.atTask) return`, כלומר F על הלפיד נפתח רק
         כשעומדים ליד *המשימה*. במחנה הלילה המשימה היא המפה שליד
         המדורה, והלפיד עומד 6.3 מטר ממנה, ליד האבן: מי שהלך אל האבן
         כפי שההוראה אמרה עמד מול לפיד שלא הגיב למקש, ומי שעמד ליד
         המפה קיבל מקש שמרים לפיד שאינו נראה. זו הייתה התחנה שהסוקר
         לא סיים ביושר. המידה הנכונה היא המרחק מן הלפיד עצמו. */
      if (Math.hypot(live.player.x - pos.current.x, live.player.z - pos.current.z) > 4.5) return
      if (document.querySelector('.hud-dialogue, .ch1-task, .ch1-find, .ch1-overlay')) return
      if (e.code === 'KeyF') {
        e.preventDefault()
...
/* ⚠ „כתובת שנחצבה" עמדה כאן כשליש מן השולחן, ומולה „מה שנחצב באבן".
   בחוברת אין כתובות: „כתובת", „חקוק", „מטבע" — אפס מופעים. שלושת
   המקורות שהיא כן מונה על מכה הם הקוראן (§44), הביוגרפיה של אבן השאם
   (§38), והמסורות החוץ־אסלאמיות שהיא קובעת שהן דלות (§33). אלה
   השלושה עכשיו, וההבדל ביניהם הוא השאלה שנשאלת אחריהם. */
const EVIDENCE_SLOTS = [
  { id: 'outside', model: 'find-sherd', h: 0.2, label: 'המסורות החוץ־אסלאמיות', place: 'מה שנמסר מחוץ לאסלאם — ודל', dx: -0.95 },
  { id: 'verse', model: 'find-scroll', h: 0.26, label: 'פסוק מן הקוראן', place: 'מה שנאמר בקוראן', dx: 0 },
  { id: 'later', model: 'prop-codex', h: 0.22, label: 'הביוגרפיה של אבן השאם', place: 'מה שנכתב דורות אחר כך', dx: 0.95 },
]
...
const RAWI_ARRIVALS: Record<string, Arrival> = {
  ...
  'border-post': {
    gesture: 'talk-nod',
    text: 'שים לב איך משתנה הדרך. מכאן והלאה יש למי לתת דין וחשבון. שני אנשים עומדים כאן: שליח של אימפריה, וראש שבט שחונה אצל המדורה.',
  },
  'narrow-pass': {
    gesture: 'talk-nod',
    text: 'המעבר הזה צר, ואין בו איש מלבדנו. כאן נעצור רגע ונבדוק מה נשאר לך משלוש התחנות שמאחורינו — ואז נמשיך.',
  },
  'loading-road': {
    gesture: 'talk',
    text: 'דרך העמסה. כאן מעבירים סחורה מגב לגב — והשאלה של התחנה היא מה עוד עבר בדרכים האלה, חוץ מסחורה. לך, ואספר תוך כדי.',
  },
  yathrib: {
    gesture: 'talk-happy',
    text: 'ית׳רב. הדקלים והבארות הם הסיבה שיושבים כאן. התחנה הזאת היא על היהודים של חצי האי — מתי הגיעו, איך חיו עם שכניהם, ומה עבר ביניהם.',
  },
  monastery: {
    gesture: 'talk-nod',
    text: 'מנזר, כאן, בקצה המדבר. התחנה הזאת היא על הנוצרים של חצי האי — איפה הם היו, ומה מהם הגיע עד מכה. מי שבחר לחיות ככה, כדאי לשמוע ממנו.',
  },
  mecca: {
    gesture: 'talk',
    text: 'ירדנו דרומה בחזרה, ומכה לפנינו. זו התחנה של עבודת האלילים — הכעבה, האבן, הפסלים — ושל סיפור אברהה. הסתובב לאט: הכל בעיר הזאת עומד במקומו מסיבה.',
  },
  exit: {
    gesture: 'talk-nod',
    text: 'זה המקום להביט אחורה. עברנו את כל הדרך — בוא נסכם אותה, תחנה אחר תחנה, ונראה מה זה אומר על העולם שערב האסלאם.',
  },
}
    ...
    <>
      ...
      {REGION_FINDS.map((fd) => (
        <GroundGlow key={`glow-${fd.id}`} x={fd.x} z={fd.z} r={0.95} live={live}
          rank={
            found.includes(fd.id) ? 'done'
            : stage === 'look' && fd.id === nextSight ? 'next'
            : stage === 'look' || stage === 'wrap' || stage === 'done' ? 'ready'
            : 'done'
          } />
      ))}
      ...
      {/* תחנת המשימה נדלקת רק כשהיא באמת הדבר הבא: בשלב הפעולה
          ובשלב הפירוש. לפני כן ואחרי כן — כבויה. */}
      {REGION_TASK && (
...
export default function Game() {
  ...
  /* ── שישה מצבים, נמדדים בנפרד ─────────────────────────────────────────
     קודם היו ארבעה, ובשבע מתשע התחנות `physDone` היה פשוט שם אחר
     ל-`taskSolved` — כלומר שלב הפעולה ושלב הפירוש היו אותו אירוע, ושלב
     „סיכום" לא התקיים בפועל. כל מצב נמדד עכשיו ממקורו:

       introSeen        שיחת הפתיחה של התחנה נשמעה
       evidenceDone     כל העדויות שהמשימה דורשת נבחנו
       physDone         הפעולה הפיזית עצמה נעשתה — לא „המשימה נפתרה"
       interpretDone    שאלת הפירוש נענתה
       summarySeen      הסיכום נאמר
       complete         כל אלה

     `physDone` נמדד מן הפעולה: בתימן זה הלפיד, במכה השולחן, ובשאר —
  ...
  useEffect(() => {
    ...
    const down = (e: KeyboardEvent) => {
      ...
      /* מקש אחד, בלי שריד.
         F נשאר עד עכשיו „קיצור סמוי לתאימות", וזה בדיוק מה שמחזיק
         שתי מערכות פעולה במקביל: סדר עדיפויות אחד ל-E ואחר ל-F,
         שתי משמעויות לאותה עמידה מול אותו חפץ, ושתי דרכים שכל בדיקה
         צריכה לכסות. הוא ירד. */
      const interact = e.code === 'KeyE'
      const examineFind = () => {
    ...
    <div className="ch1-page">
      ...
      >
        ...
        <div className={`ch1-arrive${sceneReady ? ' is-gone' : ''}`} aria-hidden={sceneReady}>
          ...
          {/* ── ומה לומדים כאן ──────────────────────────────────────
              הלוח אמר רק איפה. „התוכן לא כל כך מובן": לומד שנכנס לתחנה
              ולא יודע על מה היא — מחפש סמנים, לא נושא. הנושא הוא כותרת
              המקור עצמה, ולכן זו לא טענה שנוספה אלא שם הפרק בחוברת. */}
          {REGION.topic && <small>{REGION.topic}</small>}
        </div>
        ...
        <div className="hud-panel hud-goal">
          <span style={{ whiteSpace: 'nowrap' }}>
            {REGION.name}
            {REGION.topic && <b className="hud-topic"> · {REGION.topic}</b>}
          </span>
        ...
        {/* ⚠ התנאי כאן היה `!objective`, ושורת המטרה אינה ריקה באף שלב —
            כלומר רמז התקיעות של 25 השניות לא הוצג מעולם, מאז שנכתב.
