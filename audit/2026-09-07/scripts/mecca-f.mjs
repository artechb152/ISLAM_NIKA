...
const dir = '/private/tmp/claude-501/-Users-nikagreenbaum/9aa07fdd-34f8-43ce-9d20-79d7f178ec6f/scratchpad/video/f-mecca'
...
/* ── שולחן הראיות, במקלדת בלבד ──────────────────────────────────────
   F מרים, החצים בוחרים שקע, F מניח. יעד שגוי מחזיר את החפץ — ולכן
   מנסים שקע אחר שקע, בדיוק כמו לומד שקורא מה כתוב. */
...
for (let round=0; round<8; round++){
  ...
  for (let t=0; t<3 && !ok; t++){
    ...
    const held = (await tableAt()).drag
    if (held < 0) break
    for (let a=0; a<t; a++){ await page.keyboard.press('ArrowRight'); await page.waitForTimeout(350) }
    const hand = await page.evaluate(()=>(document.querySelector('.hud-hand')?.textContent??'').trim().slice(0,60))
    await page.keyboard.press('KeyF'); await page.waitForTimeout(850)
    ok = (await tableAt()).placed[k]
    console.log(`   מקור ${k} · ניסיון ${t+1}: ${ok?'הונח':'חזר'} · "${hand}"`)
  }
  if (!ok) break
}
