/* Chapter 4 fidelity gate. Three invariants, all machine-checked:
   ...
   Run: node concept/chapter4/verify-chapter4.mjs
*/
import { readFile } from 'node:fs/promises'

const here = new URL('.', import.meta.url)
const src = await readFile(new URL('SOURCE-TEXT.md', here), 'utf8')
const data = JSON.parse(await readFile(new URL('../../web/src/lib/chapter4/passages.json', here), 'utf8'))

let layout = null
try {
  layout = JSON.parse(await readFile(new URL('../../web/src/lib/chapter4/layout.json', here), 'utf8'))
} catch {
...
if (omitted.length) {
  console.log(`⚠ קטעים שאינם מודפסים — ממתינים להכרעה ב-DECISIONS.md: ${omitted.join(', ')}`)
}
if (reworded.length) {
