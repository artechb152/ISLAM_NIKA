...
const pages = M.panels.map((p) => ({
  ...
  b: p.beats.map((b, i) => ({
    t: b.t,
    s: b.s,
    /* ⚠ הקול נקבע מן הנתונים, לא מרשימת מזהים.
       VERSE ו-SAY היו קבוצות של מזהי קוביות, ולכן פסוק חדש שנוסף
       לקוביה שאינה ברשימה יצא בקול הקריין — כלומר בלי כרטיס הזהב
       שהפסוקים נושאים בכל הפרק. זה מה שקרה לפסוקי סורת הפיל
       שנוספו לקוביה q06 בסבב התיקונים. הרשימות נשארות כדי להבדיל
       בין פסוק לדיבור ישיר (שניהם מסומנים v בנתונים), אבל ביט
       מסומן שאינו באף אחת מהן הוא פסוק — שזה מה שהוא. */
    ...(b.v ? { k: SAY.has(p.id) ? 'say' : 'v' }
